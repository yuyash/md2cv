...
md2cv は CV コンテンツ内で標準的な Markdown 記法によるリッチテキスト書式をサポートしています。重要な実績の強調、プロジェクトへのリンク追加、技術用語のインラインコード表示などが可能です。
...
| 斜体             | `*強調*`                       | _強調_                       |
| 太字+斜体        | `***最重要***`                 | **_最重要_**                 |
| リンク           | `[GitHub](https://github.com)` | [GitHub](https://github.com) |
| インラインコード | `` `TypeScript` ``             | `TypeScript`                 |
| 取り消し線       | `~~非推奨~~`                   | ~~非推奨~~                   |

#### 使用例

概要、ハイライト、説明、リスト項目などで Markdown 書式を使用できます：

````markdown
# 概要

**フルスタックエンジニア**として*クラウドネイティブ*開発に従事。
プロジェクト例は[ポートフォリオ](https://example.com)をご覧ください。
...
- company: テック株式会社
  ...
  summary: "**重要な**インフラプロジェクトを*最新技術*でリード"
  highlights:
    - "`Node.js`と`TypeScript`で**スケーラブル**なマイクロサービスを構築"
    - "*システム信頼性*を99.9%に向上"
    - "[デザインチーム](https://design.example.com)と連携してUXを改善"
```
...
```resume:competencies
- header: 技術リーダーシップ
  description: "`Jira`と`Confluence`を活用した*アジャイル*手法で**クロスファンクショナル**チームをリード"
```
````
